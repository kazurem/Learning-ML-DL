...
def gradientDescentLogisticRegression(X, y, w, b, num_iters=100, alpha=0.00001, return_cost_history=False, regul=False,regul_type="l2", lambda_=1, verbose=False):
    '''
    X --> (m x n)
    y --> (m x 1)
    W --> (n x 1)
    ...
    for i in range(num_iters):
        ...
        dj_dw, dj_db = ML.gradients.gradientLogisticRegression(X, y, w_init, b_init, regul=regul, lambda_=lambda_, regul_type=regul_type)
        ...
        if return_cost_history == True:
            cost_history[i][0] = ML.costs.costLogisticRegression(X, y, w_init, b_init, regul=regul, lambda_=lambda_, regul_type=regul_type)
